...
def parse_ellipse_files(file, n_samples):
...
def pickle_face_data(n_samples=30000):
    faces = []
    background = []
    ...
    while len(faces)+len(background) < n_samples:
        pos, neg = parse_ellipse_files(ellipse_files[file_index], n_samples - (len(background)+len(faces)))
        faces += pos
        background += neg
        file_index += 1

    pickle_folder = get_pickle_folder()
    np.save(os.path.join(pickle_folder, 'faces'), faces)
    np.save(os.path.join(pickle_folder, 'background'), background)


def get_pickled_data(flatten=True):
    faces = np.load(os.path.join(get_pickle_folder(), 'faces.npy'))
    background = np.load(os.path.join(get_pickle_folder(), 'background.npy'))
    if flatten:
